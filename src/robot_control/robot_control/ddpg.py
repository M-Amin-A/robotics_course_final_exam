...
class ReplayBuffer:
    def __init__(self, capacity, seed=None, state_dim=None, action_dim=None):
        ...
        # Store continuous actions properly
        self.states = np.zeros((capacity, state_dim) if state_dim else (capacity,), dtype=np.float32)
        self.actions = np.zeros((capacity, action_dim) if action_dim else (capacity,), dtype=np.float32)
        self.rewards = np.zeros((capacity,), dtype=np.float32)
        self.next_states = np.zeros((capacity, state_dim) if state_dim else (capacity,), dtype=np.float32)
...
class DDPG:
    def __init__(self, env, 
        ...
        self.num_actions = 2
        self.num_states = 5
        self.observation_shape = self.num_states
        ...
        self.buffer = ReplayBuffer(
            ...
            seed=self.seed,
            state_dim=self.observation_shape,
            action_dim=self.num_actions  # Assuming num_actions is action dimension for continuous
        )
    ...
    def optimize(self):
        ...
        current_q_values = self.qvalue_net(states, actions)
        critic_loss = self.criterion(current_q_values, target_q_values)
        
        self.optimizer.zero_grad()
        critic_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.qvalue_net.parameters(), 1.0)  # Gradient clipping
        self.optimizer.step()
        
        # Actor update
        actor_actions = self.actor(states)
        actor_loss = -self.qvalue_net(states, actor_actions).mean()
